...
class CheckOut(PermissionRequiredMixin, FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if 'cart_products' in request.session.keys():
            ...
            urlbase = "https://development.weberp.co.tz/v1/gateway/get-token"
            registration = {'companyID':'companyID' , 'TIN':'TIN' , 'serail_number':'serial_number'}
            response = requests.post(urlbase, headers={'Content-Type':'application/json'}, data=json.dumps(registration))
            if response.status_code == 201:
                data = response.json()
                return HttpResponse(data , status=response.status_code)
            else:
                return HttpResponse({"error": "Method not allowed"}, status=response.status_code)
...
# def CompanyRegistraion(request):
...
def create_company_registration(request):
        ...
        password = ""
        registration = {'companyID':'companyID' , 'TIN':'TIN' , 'serail_number':'serial_number'}
        response = requests.post(urlbase, headers={'Content-Type':'application/json'}, data=json.dumps(registration), auth=(username, password))
        if response.status_code == 201:
            data = response.json()
            return HttpResponse(data , status=response.status_code)
